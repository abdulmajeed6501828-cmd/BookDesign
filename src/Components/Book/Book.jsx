...
const Book = () => {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        setIsBookReady(true);
        // Force book to page 0 after it's ready
        setTimeout(() => {
  ...
  const handleNavClick = (e, section) => {
    ...
    // Only allow navigation if not in landing state
    if (isLanding) return;
    const targetPage = SECTION_PAGES[section];
    if (bookRef.current && bookRef.current.pageFlip()) {
      bookRef.current.pageFlip().flip(targetPage);
  ...
  const handlePointerDown = (e) => {
    if (isLanding) return; // Prevent interaction during landing
    if (activePage === 1 && e.clientX < window.innerWidth / 2) {
      setIsClosing(true);
    } else if (activePage === pages.length - 2 && e.clientX > window.innerWidth / 2) {
      setIsClosing(true);
  ...
  const handleFlip = (e) => {
    // Prevent page flip during landing state
    if (isLanding) {
      // Force the book back to page 0 if it tries to flip
      if (bookRef.current && bookRef.current.pageFlip()) {
  ...
  // Function to open the book (triggered by intended interaction)
  const openBook = () => {
    setIsLanding(false);
    // Optionally flip to page 1 when opening
    if (bookRef.current && bookRef.current.pageFlip()) {
      setTimeout(() => {
  ...
  // Calculate transform based on landing state
  const getTransform = () => {
    if (isLanding) {
      ...
      <div className={`book-stage-area ${isLanding ? 'is-landing-pos' : ''}`}>
          <div 
            ...
            style={{
              ...
              transition: isBookReady ? "transform 1s cubic-bezier(0.645, 0.045, 0.355, 1)" : "none",
              opacity: isBookReady && isFlipbookMounted ? 1 : 0,
              visibility: isBookReady && isFlipbookMounted ? 'visible' : 'hidden',
              // Prevent any layout shift during initial render
          ...
          >
            <div style={{ 
              position: "relative", 
              width: portrait ? width : width * 2, 
              height: height, 
              pointerEvents: 'auto',
              overflow: 'hidden',
              // Ensure dimensions are stable
              minWidth: portrait ? width : width * 2,
              minHeight: height,
              maxWidth: portrait ? width : width * 2,
              maxHeight: height,
              ...
              {isLanding && (
                <div
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    // Don't open the book - this click should do nothing
                  }}
                  style={{
                    position: "absolute",
                    inset: 0,
              ...
              <HTMLFlipBook
                ...
                showPageCorners={false}
                mobileScrollSupport={true}
                clickEventForward={!isLanding}
                useMouseEvents={!isLanding}
                onFlip={handleFlip}
                onChangeState={handleStateChange}
                className="html-book"
                ref={bookRef}
